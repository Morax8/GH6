...
export default function CultureGuessr() {
  ...
  const currentBuilding =
    currentBuildingIndex < CulturalBuildings.length
      ? CulturalBuildings[currentBuildingIndex]
      : null;

  const isGameOver =
    timeLeft <= 0 || currentBuildingIndex >= CulturalBuildings.length;
  ...
  useEffect(() => {
    ...
    if (!document.getElementById(scriptId)) {
      ...
      script.id = scriptId;
      script.src =
        "https://maps.googleapis.com/maps/api/js?key=AIzaSyDq7QPD1y02MkjNfGunkeLXEbGB4-SxlqM";
      script.async = true;
      script.defer = true;
  ...
  const handleGuessSubmit = (e) => {
    if (e.key === "Enter" && currentBuilding) {
  ...
  const visibleHints =
    currentBuilding?.hints?.filter((_, index) => {
      ...
      <div className="flex flex-col gap-8 justify-center items-center h-200 col-start-10 col-span-3">
        {isGameOver ? (
          <div className="flex flex-col h-120 w-full gap-4">
            ...
            <div className="font-bold text-5xl text-center">GAME OVER</div>
        ...
        {!isGameOver && (
          <div className="flex flex-col w-full gap-2">
            ...
            <input
              ...
              className={`w-full bg-[#EDEDED] py-3 px-5 rounded-full outline-none border ${
                isCorrect === null
                  ? "border-transparent"
                  : isCorrect
                  ? "border-green-500"
